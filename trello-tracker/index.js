...
function createEmbed(changeType, data) {
	...
	if (changeType === "card_created") {
		...
		if (data.desc) {
			embed.fields.push({
				name: "Description",
				value: data.desc.substring(0, 1024),
			});
		}
		if (data.coverImage) {
	...
	if (changeType === "card_updated") {
		...
		if (data.changes.labels) {
			const oldLabels =
				data.changes.labels.old.map((l) => l.name).join(", ") || "None";
			const newLabels =
				data.changes.labels.new.map((l) => l.name).join(", ") || "None";
			embed.fields.push({
				name: "Labels Changed",
...
async function detectChanges(oldState, newBoard) {
	...
	for (const card of newBoard.cards) {
		...
		const listName =
			newBoard.lists.find((l) => l.id === card.idList)?.name || "Unknown";
		...
		if (!oldState.cards[card.id]) {
			...
			// attachment changes
			const oldAttachmentsMap = new Map(
				(oldCard.attachments || []).map((a) => [a.id, a]),
			);
			const newAttachmentsMap = new Map(
				(card.attachments || []).map((a) => [a.id, a]),
			);
	...
	return { newCards, newLists, changes };
}
